...
case class CsvRelation protected[spark] (
  ...
  private val logger = LoggerFactory.getLogger("Rejection_Error_Logger")
  private val rejectLogger = LoggerFactory.getLogger("Rejected_Rows_Logger")
  ...
  override def buildScan: RDD[Row] = {
    ...
    tokenRdd(schemaFields.map(_.name)).flatMap { tokens =>
      ...
      if (dropMalformed && schemaFields.length != tokens.length) {
        rejectLogger.warn(s"${tokens.mkString(delimiter.toString)}" + s"${delimiter.toString}" + s"Schema length mismatch")
        None
        ...
        rejectLogger.warn(s"${tokens.mkString(delimiter.toString)}" + s"${delimiter.toString}" + s"Schema type mismatch in column #${index}")
            None
        }
      }
    }
  ...
  override def buildScan(requiredColumns: Array[String]): RDD[Row] = {
	...
	buildScan()
      ...
      tokenRdd(schemaFields.map(_.name)).flatMap { tokens =>
        if (dropMalformed && schemaFields.length != tokens.length) {
        rejectLogger.info(s"${tokens.mkString(delimiter.toString)}" + s"${delimiter.toString}" + s"Schema length mismatch")
          None
        } else if (failFast && schemaFields.length != tokens.length) {
          ...
          try {
            ...
            case _: java.lang.NumberFormatException |
                 _: IllegalArgumentException if dropMalformed=>
            rejectLogger.info(s"${tokens.mkString(delimiter.toString)}" + s"${delimiter.toString}" + s"Schema type mismatch in column #${index}")
              None
          }
        }
      }
